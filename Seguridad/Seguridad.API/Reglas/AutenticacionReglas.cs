    ...
    public class AutenticacionReglas : IAutenticacionBC
    {
        ...
        public async Task<Token> LoginAync(LoginBase login)
        {
            ...
            if (!resultadoVerificacionCredenciales)
                return respuestaToken;
            TokenConfiguracion tokenConfiguracion=_configuration.GetSection("Token").Get<TokenConfiguracion>();//Obtener la configuración del token desde el appsettings.json
